   ...
   for g = 1:num_groups
      if ~iscell(num_subj_lst)
         ...
         if exist('weights','var')
             weights = weights(datamat_reorder);
         end
         ...
         if exist('weights','var')
             weights_dat = weights(datamat_reorder);
             weights_dat = weights(1+span:n*k+span);
             weights_beh = weights(behavdata_reorder);
             weights_beh = weights(1+span:n*k+span);
         end
